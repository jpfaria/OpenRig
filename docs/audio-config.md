...
A chain's DI loop plays on its **own isolated, pre-rendered stream** (#771) — it never replaces or rides the guitar's live input. Arming resolves the chain's persisted output choice (`Chain.di_output` → one of its bound output endpoints; absent → the main output), pre-renders the loop through a fresh copy of the chain's block graph on a short-lived `di-render` thread at that output's rate (two full periods are rendered and the steady-state second is kept, so wrap tails loop seamlessly), and parks the buffer on that output's playback cell. The output device's callback then sums the buffer at a cursor it advances itself — output-clocked by construction (the free-running worker→output routing tried in #717 drifted against the device clock and was reverted), zero allocation/locks/DSP in the callback. Decoding, resampling (per-output rate, #749) and rendering all happen off the audio thread. The guitar runtime, its meters, and every other output are untouched (isolation invariant #4); a device-rate change re-renders at the new rate (#669). The render is a snapshot of the chain: block edits while playing apply on the next play. The **source** choice stays runtime-only; only the **output** choice persists, inside the chain in `project.openrig` (ADR 0003). See the **Virtual DI loop** entry under **Chains** in `docs/screens.md` for UI details.
