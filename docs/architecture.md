...
- `adapter-render` — headless offline render driver (issue #552). Binary `openrig-render` and lib `adapter_render::render()`. Loads a project, decodes an input WAV, drives `engine::offline::render_chain` (same `RuntimeProcessor::process_buffer` as the realtime callback), writes the output WAV atomically. No cpal, no Slint, no MCP, no MIDI — single-chain, deterministic, used by the audio-validation pipeline (`openrig-tone-analyzer` skill, OpenRig-claude#8). The GUI binary forwards to it when `--render` is on argv.
