...
- **Block Editor** — edita parâmetros de um bloco. The header's model select/search picker is shown only for catalog-backed block types; the generic NAM (`nam`) and IR (`ir`) file-loader blocks have no catalog model to choose, so their header shows the loaded model name as a static label instead of the picker (#608). The window height is capped (`MAX_PANEL_HEIGHT_PX`); plugins with enough parameters to overflow that cap keep their wrapped knob grid in a scroll view instead of growing the window past the screen (#622).
