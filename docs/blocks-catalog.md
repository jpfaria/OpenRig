...
- **NAM / Amp / Preamp (`neural_amp_modeler`)**: `input_db` / `output_db` (gain staging, applied), `eq.enabled` + `eq.bass` / `eq.middle` / `eq.treble` (0–10, 5 = flat) — a post-model 3-band tone stack (low shelf @120 Hz, mid peak @750 Hz, high shelf @4 kHz, ±12 dB at the extremes; transparent and bypassed at 5/5/5). `noise_gate.*` is currently a no-op (the static expander was reverted in #496 because it strangled guitar dynamics — see `crates/nam/src/processor.rs`). EQ wired in #612.
