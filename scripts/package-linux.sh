#!/usr/bin/env bash
# Build Linux packages (.deb, .tar.gz, .rpm, .AppImage) for a given arch.
#
# Runs natively on Linux — used by GitHub Actions and by build-linux-local.sh
# (which wraps it in Docker for macOS use).
#
# Usage:
#   ./scripts/package-linux.sh [--arch arm64|x86_64] [--version V]
#
# Output (in dist/):
