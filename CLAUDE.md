...
1. **DOIS comandos primeiro, cada um no seu bloco de código, sempre os dois:** pasta principal — `git fetch && git checkout {tipo}/issue-N && git pull`; pasta solver — o comando **COMPLETO, com caminho ABSOLUTO**, exatamente a linha `run:` que `scripts/solver-setup.sh` imprime (`cd /Users/…/OpenRig/.solvers/issue-N && OPENRIG_PLUGINS_ROOT=<plugins_path> cargo run -p adapter-gui -- --mcp`, + qualquer env var que o teste precise; `OPENRIG_PLUGINS_ROOT` ganha de qualquer config, então os plugins carregam sempre). Faltou um dos dois, ou caminho relativo (`cd .solvers/…`) = checklist incompleto. **O workspace do agente SEMPRE nasce por `scripts/solver-setup.sh <N> <branch> [release-base]`** — clone real (o NAM vem do arquivo LFS, #974) + link `plugins` pro `paths.plugins_path` do config (sem o link o app abre com zero plugins, #938). Workspace montado à mão? Rode o script em cima antes de entregar o checklist: ele completa o que falta e falha alto se não achar os plugins.
...
**Why:** ele precisa do checklist toda entrega e cansou de pedir (#959: teve que cobrar de novo o comando completo e os plugins — por isso virou script); sem os dois comandos + os checkboxes ele não consegue puxar e validar rápido — e ele escolhe na hora se valida pela pasta principal (RustRover) ou direto no solver (#934: entregue só o comando da pasta principal, ele teve que pedir o outro). Uma resposta de "9 frases" NÃO é checklist — checkbox é checkbox.
...
- **Antes de TODO push: `cargo test --workspace`, nunca por crate.** `cargo test -p <crate> --lib` compila só a lib daquele crate — um arquivo quebrado em `tests/` (ou um módulo de teste que só existe atrás de um `cfg`) passa batido e o CI reprova. Já quebrou o CI duas vezes na mesma branch: um teste-sonda que chamava função inexistente, e um teste que usava fixtures cfg-adas fora do `linux+jack`. O comando é `cargo fmt --all -- --check` (limpo) + `cargo test --workspace` + `cargo build --workspace` (zero warnings) + `VALIDATE_STATIC_ONLY=1 ./scripts/validate.sh crates` (o repo INTEIRO), e só então o push. **Sem o `cargo fmt --check` o gate do PR `release → main` reprova na métrica `fmt`** — foi o que segurou a entrega da v0.4.3: 6 trechos sem formatar vindos da #938 passaram no PR da issue e só apareceram no PR da release. **`./scripts/validate.sh $(git diff --name-only HEAD)` não serve como gate de push:** esse `git diff` lista só o que está NÃO-commitado, então depois do commit ele valida uma lista vazia e passa sempre — foi assim que a #913 subiu com duas declarações de responsabilidade inválidas e reprovou no job `Static Checks (whole repo)`. **Warning conta como quebrado:** import não usado, `mut` desnecessário, dead code — se apareceu no build, some antes do push.
