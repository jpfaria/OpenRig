...
**UI/Slint.** **OBRIGATÓRIO invocar a skill `ux-ui-mastery` ANTES de qualquer trabalho de tela/layout (`.slint`, posicionamento, espaçamento, hierarquia, componente visual) — PROIBIDO supor/inventar layout, o agente não renderiza a tela. Fechamento visual em loop curto com o usuário. Detalhe na LEI do `openrig-code-quality`.** Nunca glifo como ícone (vira tofu no Orange Pi) — sempre SVG via `@image-url` + colorize. Bebas Neue é a fonte default por escolha — não propor trocar. Manter consistência visual cross-screen.
