//! Responsibility: registers the body models this crate exposes.
...
#[derive(Clone, Copy)]
#[allow(dead_code)]
pub struct BodyModelDefinition {
    pub id: &'static str,
    pub display_name: &'static str,
    pub brand: &'static str,
    pub backend_kind: BodyBackendKind,
...
pub fn find_model_definition(model: &str) -> Result<&'static BodyModelDefinition> {
    MODEL_DEFINITIONS
        .iter()
        .find(|definition| definition.id == model)
        .ok_or_else(|| anyhow!("unsupported body model '{}'", model))
}
