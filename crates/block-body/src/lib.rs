...
mod tests {
    ...
    use crate::{
        build_body_processor_for_layout, body_model_schema, supported_models,
        body_display_name, body_brand, body_type_label, body_backend_kind,
        body_model_visual, body_asset_summary, validate_body_params,
        BodyBackendKind,
    };
    ...
    fn all_models_return_valid_schema() {
        for model in supported_models() {
            ...
            assert!(schema.is_ok(), "schema() failed for model '{model}': {}", schema.unwrap_err());
            let schema = schema.unwrap();
            assert_eq!(schema.model, *model, "schema model id mismatch for '{model}'");
            assert!(!schema.parameters.is_empty(), "model '{model}' should have parameters");
        }
    }
    ...
    fn all_models_validate_with_defaults() {
        for model in supported_models() {
            ...
            assert!(result.is_ok(), "validate failed for model '{model}' with defaults: {}", result.unwrap_err());
        }
    }
    ...
    fn all_models_have_ir_backend() {
        for model in supported_models() {
            ...
            assert_eq!(kind, BodyBackendKind::Ir, "body model '{model}' should be IR backend");
        }
    }
    ...
    fn all_models_type_label_is_ir() {
        for model in supported_models() {
            ...
            assert_eq!(label, "IR", "body model '{model}' type_label should be 'IR', got '{label}'");
        }
    }
    ...
    fn all_models_return_visual_data() {
        for model in supported_models() {
            ...
            assert!(visual.is_some(), "model '{model}' should return visual data");
            let visual = visual.unwrap();
            assert_eq!(visual.type_label, "IR");
            assert!(!visual.supported_instruments.is_empty(),
    ...
    fn all_models_return_asset_summary_with_defaults() {
        for model in supported_models() {
            ...
            assert!(summary.is_ok(), "asset_summary failed for model '{model}': {}", summary.unwrap_err());
            assert!(!summary.unwrap().is_empty(), "asset_summary should not be empty for '{model}'");
        }
    }
