...
pub const DISPLAY_NAME: &str = "Mud Filter";
...
fn build_mono(sample_rate: f32, mix: f32, filter: f32, lfo: f32) -> Result<lv2::Lv2Processor> {
    let lib_path = lv2::resolve_lv2_lib(PLUGIN_BINARY)?;
    let bundle_path = lv2::resolve_lv2_bundle(PLUGIN_DIR)?;
    lv2::build_lv2_processor(
        &lib_path, PLUGIN_URI, sample_rate as f64, &bundle_path,
        &[PORT_AUDIO_IN], &[PORT_AUDIO_OUT],
        &[(PORT_MIX, mix), (PORT_FILTER, filter), (PORT_LFO, lfo)],
    )
}
...
fn build(params: &ParameterSet, sample_rate: f32, layout: AudioChannelLayout) -> Result<BlockProcessor> {
    ...
    match layout {
        AudioChannelLayout::Mono => {
            Ok(BlockProcessor::Mono(Box::new(build_mono(sample_rate, mix, filter, lfo)?)))
        }
        AudioChannelLayout::Stereo => {
            let left = build_mono(sample_rate, mix, filter, lfo)?;
            let right = build_mono(sample_rate, mix, filter, lfo)?;
