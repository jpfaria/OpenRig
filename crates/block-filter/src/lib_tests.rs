...
fn native_filter_build_mono_at_44100() {
    for model in native_filter_models() {
        ...
        let result =
            build_filter_processor_for_layout(model, &params, 44_100.0, AudioChannelLayout::Mono);
        assert!(
            result.is_ok(),
...
fn native_filter_stereo_build_rejected() {
    ...
    for model in native_filter_models() {
        let params = default_params_for(model);
        let result =
            build_filter_processor_for_layout(model, &params, 44_100.0, AudioChannelLayout::Stereo);
