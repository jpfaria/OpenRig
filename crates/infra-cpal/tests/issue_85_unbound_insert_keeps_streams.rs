//! Issue #85 — an `Insert` block added but not yet wired to an E/S silenced the
//! chain: its input meter fell to -120 dBFS and only came back when the block
//! was removed. The engine itself plays fine with an unbound port, so the
//! question is whether the CONTROLLER still opens the streams for such a chain.
//!
...
fn disabling_and_re_enabling_a_chain_brings_the_audio_back() {
    ...
    let peak = listener.measure(3);
    eprintln!("[#85 INSERT] unbound Insert: {streams} stream(s), peak {peak:.3}");
    assert!(
        streams > 0,
        "#85: a chain carrying an unwired Insert opened no stream at all"
    );
    assert!(
        peak > 0.01,
        "#85: an Insert with no E/S picked yet silenced the chain (peak {peak:.3}) — \
         an unwired port must sit out, not take the rig down"
    );
}
