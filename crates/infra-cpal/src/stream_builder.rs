    ...
    let stream = match sample_format {
        SampleFormat::F32 => {
            ...
            device.build_input_stream(
                &stream_config,
                move |data: &[f32], _| {
                    // Issue #670: promote THIS audio callback thread to a
                    // real-time time-constraint policy on the first call, so
                    // the Slint UI thread (render + spectrum FFT + meters) can
                    // not preempt it past its 64-frame deadline (the crackle).
                    if !rt_promoted {
                        rt_promoted = true;
