...
fn supported_delay_models_build_for_stereo_chains() {
    ...
    for model in native_delay_models() {
        ...
        let processor =
            build_delay_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Stereo);
...
fn supported_delay_models_build_for_mono_chains() {
    ...
    for model in native_delay_models() {
        ...
        let params = ParameterSet::default()
            ...
            .expect("normalized defaults");
        let processor =
            build_delay_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Mono);
...
fn native_delay_process_silence_stereo_produces_finite() {
    for model in native_delay_models() {
        let params = default_params_for(model);
        let mut processor =
            build_delay_processor_for_layout(model, &params, 44100.0, AudioChannelLayout::Stereo)
                .expect("build");
