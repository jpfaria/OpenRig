...
use processor::{
    ...
    plugin_params_from_set, plugin_params_from_set_with_defaults, supports_model, NamPluginParams,
...
fn model_schema_with_file_params_includes_model_path() {
    ...
    assert!(schema.parameters.iter().any(|p| p.path == "model_path"));
}

#[test]
fn model_schema_with_file_params_includes_ir_path() {
    let schema = model_schema(true);
    assert!(schema.parameters.iter().any(|p| p.path == "ir_path"));
}
...
fn params_from_set_with_model_path_succeeds() {
    ...
    ps.insert(
        "model_path",
        ParameterValue::String("/path/to/model.nam".into()),
    );
...
fn params_from_set_with_ir_path_returns_some() {
    ...
    ps.insert(
        "model_path",
        ParameterValue::String("/path/model.nam".into()),
    );
...
fn build_processor_nonexistent_model_returns_error() {
    ...
    ps.insert(
        "model_path",
        ParameterValue::String("/nonexistent.nam".into()),
    );
...
fn build_processor_for_layout_stereo_returns_error() {
    ...
    let result = build_processor_for_layout(&ps, 48000.0, block_core::AudioChannelLayout::Stereo);
    assert!(result.is_err());
}
