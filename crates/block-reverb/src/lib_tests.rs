...
fn native_reverb_build_mono_with_defaults() {
    for model in native_reverb_models() {
        ...
        let result =
            build_reverb_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Mono);
        assert!(
            result.is_ok(),
...
fn native_reverb_stereo_capable_models_build_stereo() {
    ...
    for model in stereo_models {
        ...
        let params = ParameterSet::default()
            ...
            .expect("normalized defaults");
        let result =
            build_reverb_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Stereo);
        assert!(
            result.is_ok(),
...
fn native_reverb_build_mono_at_44100() {
    for model in native_reverb_models() {
        ...
        let params = ParameterSet::default()
            ...
            .expect("normalized defaults");
        let result =
            build_reverb_processor_for_layout(model, &params, 44_100.0, AudioChannelLayout::Mono);
        assert!(
            result.is_ok(),
...
fn native_reverb_stereo_capable_build_stereo_at_44100() {
    ...
    for model in stereo_models {
        ...
        let params = ParameterSet::default()
            ...
            .expect("normalized defaults");
        let result =
            build_reverb_processor_for_layout(model, &params, 44_100.0, AudioChannelLayout::Stereo);
        assert!(
            result.is_ok(),
