    ...
    fn default_reverb() -> CathedralReverb {
        CathedralReverb::new(Params::default(), 44_100.0)
    }

    #[test]
    fn hadamard16_preserves_energy() {
        let mut x: [f32; N] = std::array::from_fn(|i| (i as f32 + 1.0) * 0.1);
        let energy_in: f32 = x.iter().map(|v| v * v).sum();
        hadamard16(&mut x);
        let energy_out: f32 = x.iter().map(|v| v * v).sum();
        assert!((energy_in - energy_out).abs() < 1e-4, "{} != {}", energy_in, energy_out);
    }

    #[test]
    fn hadamard16_self_inverse_up_to_scale() {
        let original: [f32; N] = std::array::from_fn(|i| (i as f32 + 1.0) * 0.1);
        let mut x = original;
        hadamard16(&mut x);
        hadamard16(&mut x);
        for i in 0..N {
            assert!((x[i] - original[i]).abs() < 1e-3, "{}: {} != {}", i, x[i], original[i]);
        }
    }
    ...
    fn mono_adapter_runs_silence_and_sine() {
        let mut mono = CathedralAsMono(default_reverb());
        for _ in 0..512 {
