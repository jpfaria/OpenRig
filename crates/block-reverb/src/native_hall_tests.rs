...
#[test]
fn hall_output_is_finite() {
    let mut reverb = HallReverb::new(Params::default(), 44_100.0);
    for i in 0..10_000 {
...
fn process_frame_mono_adapter_silence_finite() {
    let mut mono = HallAsMono(HallReverb::new(Params::default(), 44_100.0));
    for i in 0..1024 {
...
fn process_frame_mono_adapter_sine_finite_and_nonzero() {
    ...
    assert!(
        any_nonzero,
        "expected non-zero output for sine input (mono)"
    );
}
