...
fn build_at_various_sample_rates() {
    ...
    for &sr in &[44100.0_f32, 48000.0, 88200.0, 96000.0] {
        assert!(
            build(&params, sr, AudioChannelLayout::Mono).is_ok(),
            "build failed at {sr}"
        );
    }
}
...
fn process_sine_produces_nonzero_output_at_default_volume() {
    ...
    assert!(
        out_rms > 0.01,
        "default volume should produce audible output, rms={out_rms}"
    );
}
...
fn mute_true_produces_silence() {
    ...
    for (i, &s) in input.iter().enumerate() {
        ...
        assert_eq!(
            out, 0.0,
            "mute=true should produce silence, got {out} at sample {i}"
        );
    }
}
...
fn mute_false_passes_signal() {
    ...
    assert!(
        out_rms > 0.01,
        "mute=false should pass signal, rms={out_rms}"
    );
}
...
fn percent_to_db_80_is_near_unity() {
    ...
    assert!(
        db > -5.0 && db < 0.5,
        "80% should be near unity, got {db}dB"
    );
}
...
fn percent_to_db_monotonically_increasing() {
    ...
    for pct in (1..=100).step_by(1) {
        ...
        assert!(
            db >= prev,
            "percent_to_db should be monotonic: {prev} -> {db} at {pct}%"
        );
        prev = db;
    }
}
...
fn volume_processor_is_linear_when_not_muted() {
    ...
    assert!(
        ...
        "volume should be linear: out(1.0)={out2}, 2*out(0.5)={}",
        2.0 * out1
    );
}
