...
fn schema_audio_mode_is_dual_mono() {
    assert_eq!(
        model_schema().audio_mode,
        block_core::ModelAudioMode::DualMono
    );
}
...
fn process_silence_produces_near_zero_output() {
    ...
    assert!(
        avg < 1e-6,
        "silence should produce near-zero output, got avg={avg}"
    );
}
...
fn process_mono_sine_produces_nonzero_output() {
    ...
    assert!(
        out_rms > 0.001,
        "expected nonzero output, got rms={out_rms}"
    );
}
...
fn tone_variation_produces_different_spectral_balance() {
    ...
    let out_bright: Vec<f32> = input
        .iter()
        .map(|&s| proc_bright.process_sample(s))
        .collect();
...
fn soft_clip_small_values_nearly_linear() {
    ...
    assert!(
        error < 0.01,
        "soft_clip should be nearly linear for small values, error={error}"
    );
}
...
fn soft_clip_at_clamp_boundary_is_deterministic() {
    ...
    assert!(
        ...
        "soft_clip(10) should be {expected}, got {pos}"
    );
    assert!(
        (neg - (-expected)).abs() < 1e-6,
        "soft_clip(-10) should be {}, got {neg}",
        -expected
    );
}
...
fn soft_clip_within_range_is_bounded() {
    ...
    for &x in &[0.5, 0.8, 1.0] {
        ...
        assert!(
            out <= x,
            "soft_clip({x}) should not exceed input, got {out}"
        );
    }
}
...
fn golden_sample_mono_defaults_48khz() {
    ...
    let output2: Vec<f32> = test_input2
        .iter()
        .map(|&s| proc2.process_sample(s))
        .collect();
...
fn extreme_params_all_max_produces_finite() {
    ...
    for (i, &s) in input.iter().enumerate() {
        ...
        assert!(
            out.is_finite(),
            "all-max params produced non-finite at {i}: {out}"
        );
    }
}
...
fn extreme_params_all_min_produces_finite() {
    ...
    for (i, &s) in input.iter().enumerate() {
        ...
        assert!(
            out.is_finite(),
            "all-min params produced non-finite at {i}: {out}"
        );
    }
}
...
fn hot_input_signal_produces_finite_output() {
    ...
    for i in 0..512 {
        ...
        assert!(
            out.is_finite(),
            "hot input produced non-finite at {i}: {out}"
        );
    }
    ...
    for i in 0..256 {
        ...
        assert!(
            out.is_finite(),
            "overdriven input produced non-finite at {i}: {out}"
        );
    }
}
