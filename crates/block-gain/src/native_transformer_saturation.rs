//! Responsibility: implements the transformer saturation gain model.
...
struct DualMonoProcessor {
    left: TransformerProcessor,
    right: TransformerProcessor,
}
...
fn build(p: &ParameterSet, sample_rate: f32, layout: AudioChannelLayout) -> Result<BlockProcessor> {
    ...
    Ok(match layout {
        AudioChannelLayout::Mono => {
            BlockProcessor::Mono(Box::new(TransformerProcessor::new(s, sample_rate)))
        }
