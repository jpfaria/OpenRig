    ...
    fn process_sample_silence_output_finite() {
        let mut vib = Vibrato::new(4.0, 0.5, 44_100.0);
        for i in 0..1024 {
            let out = MonoProcessor::process_sample(&mut vib, 0.0);
    ...
    fn process_sample_sine_output_finite_and_nonzero() {
        ...
        for i in 0..1024 {
            ...
            let out = MonoProcessor::process_sample(&mut vib, input);
            assert!(out.is_finite(), "output not finite at sample {i}");
            if out.abs() > 1e-10 {
                any_nonzero = true;
            }
    ...
    fn process_block_all_finite() {
        ...
        let mut buffer: Vec<f32> = (0..1024)
            ...
            .collect();
        MonoProcessor::process_block(&mut vib, &mut buffer);
    ...
    fn process_sample_silence_is_zero() {
        ...
        for _ in 0..1024 {
            let out = MonoProcessor::process_sample(&mut vib, 0.0);
            assert_eq!(out, 0.0, "vibrato of silence should be silence");
        }
    }
