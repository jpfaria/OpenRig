...
fn build_output_routing_state_mono_single_channel() {
    ...
    let state = build_output_routing_state(&output, DEFAULT_ELASTIC_TARGET, 0, 48_000.0, true);
...
fn build_output_routing_state_mono_mode_with_two_channels_uses_mono() {
    ...
    let _state = build_output_routing_state(&output, DEFAULT_ELASTIC_TARGET, 0, 48_000.0, true);
