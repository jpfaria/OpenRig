...
pub(crate) fn wire(
    ...
    // Project device toggles & value updates — standalone ProjectSettingsWindow.
    {
        ...
        crate::SettingsBridge::get(project_settings_window).on_toggle_project_device(move |index, selected| {
            toggle_device_row(&project_devices, index as usize, selected);
        });
    }
    {
        let project_devices = project_devices.clone();
        crate::SettingsBridge::get(project_settings_window).on_update_project_sample_rate(move |index, value| {
            update_device_sample_rate(&project_devices, index as usize, value);
        });
    }
    {
        let project_devices = project_devices.clone();
        crate::SettingsBridge::get(project_settings_window).on_update_project_buffer_size(move |index, value| {
            update_device_buffer_size(&project_devices, index as usize, value);
        });
    }
    {
        let project_devices = project_devices.clone();
        crate::SettingsBridge::get(project_settings_window).on_update_project_bit_depth(move |index, value| {
...
#[cfg(test)]
#[path = "device_settings_wiring_tests.rs"]
mod tests;
