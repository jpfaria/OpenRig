...
pub fn run_desktop_app(
    ...
    // Startup audio — if the loaded project already has enabled chains (e.g.
    // the kiosk auto-opened /etc/openrig.yaml with enabled=true), kick off
    // JACK and connect chains once the event loop is running.
    {
        let project_session_startup = project_session.clone();
        let project_runtime_startup = project_runtime.clone();
        let project_chains_startup = project_chains.clone();
        let input_chain_devices_startup = input_chain_devices.clone();
        let output_chain_devices_startup = output_chain_devices.clone();
        let weak_window_startup = window.as_weak();
        let toast_timer_startup = toast_timer.clone();
        let startup_timer = slint::Timer::default();
        startup_timer.start(
            slint::TimerMode::SingleShot,
            std::time::Duration::from_millis(500),
            move || {
                let session_borrow = project_session_startup.borrow();
                let Some(session) = session_borrow.as_ref() else {
                    log::warn!("startup timer: no project session loaded yet");
                    return;
                };
                ...
                #[cfg(target_os = "linux")]
                {
                    ...
                    if let Some(win) = weak_window_startup.upgrade() {
                        set_status_info(&win, &toast_timer_startup, "Starting audio...");
                    }
                    log::info!("startup: kicking off audio for pre-enabled chains");

                    let rx = Rc::new(std::cell::RefCell::new(
                        infra_cpal::start_jack_in_background(sample_rate, buffer_size),
                    ));
                    let project_session_t = project_session_startup.clone();
                    let project_runtime_t = project_runtime_startup.clone();
                    let project_chains_t = project_chains_startup.clone();
                    let input_chain_devices_t = input_chain_devices_startup.clone();
                    let output_chain_devices_t = output_chain_devices_startup.clone();
                    let weak_win_t = weak_window_startup.clone();
                    let toast_timer_t = toast_timer_startup.clone();
                    ...
                    poll_timer.start(
                        ...
                        move || {
                            ...
                            match rx.borrow().try_recv() {
                                ...
                                Ok(Err(e)) => {
                                    done_t.set(true);
                                    if let Some(w) = weak_win_t.upgrade() {
                                        set_status_error(&w, &toast_timer_t, &e.to_string());
                                    }
                                }
                                Ok(Ok(())) => {
                                    done_t.set(true);
                                    let Some(w) = weak_win_t.upgrade() else { return; };
                                    let mut sb = project_session_t.borrow_mut();
                                    let Some(s) = sb.as_mut() else { return; };
                                    if let Err(e) = sync_project_runtime(&project_runtime_t, s) {
                                        set_status_error(&w, &toast_timer_t, &e.to_string());
                                    } else {
                                        replace_project_chains(&project_chains_t, &s.project, &*input_chain_devices_t.borrow(), &*output_chain_devices_t.borrow());
                                        set_status_info(&w, &toast_timer_t, "");
                                        log::info!("startup: audio engine started for pre-enabled chains");
                                    }
                                }
                            }
                        },
                ...
                {
                    drop(session_borrow);
                    let mut sb = project_session_startup.borrow_mut();
                    let Some(s) = sb.as_mut() else { return; };
                    if let Err(e) = sync_project_runtime(&project_runtime_startup, s) {
                        if let Some(w) = weak_window_startup.upgrade() {
                            set_status_error(&w, &toast_timer_startup, &e.to_string());
                        }
                    } else {
                        replace_project_chains(&project_chains_startup, &s.project, &*input_chain_devices_startup.borrow(), &*output_chain_devices_startup.borrow());
                    }
                }
            },
        );
        std::mem::forget(startup_timer);
    }
