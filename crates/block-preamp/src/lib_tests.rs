...
fn registry_build_native_models_mono() {
    for model in supported_models().iter().filter(|m| is_native(m)) {
        let params = defaults_for(model);
        let processor =
            build_preamp_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Mono)
...
fn registry_build_native_models_stereo() {
    for model in supported_models().iter().filter(|m| is_native(m)) {
        let params = defaults_for(model);
        let processor =
            build_preamp_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Stereo)
