...
fn registry_build_native_models_mono() {
    for model in supported_models().iter().filter(|m| is_native(m)) {
        let params = defaults_for(model);
        let processor =
            build_amp_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Mono)
...
fn registry_build_native_models_stereo() {
    for model in supported_models().iter().filter(|m| is_native(m)) {
        let params = defaults_for(model);
        let processor =
            build_amp_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Stereo)
...
fn registry_process_native_mono_block_1024_silence_all_finite() {
    for model in supported_models().iter().filter(|m| is_native(m)) {
        let params = defaults_for(model);
        let mut proc =
            match build_amp_processor_for_layout(model, &params, 44100.0, AudioChannelLayout::Mono)
                .unwrap()
            {
                BlockProcessor::Mono(p) => p,
                BlockProcessor::Stereo(_) => panic!("expected Mono for '{model}'"),
...
fn registry_build_non_native_models_ignored() {
    for model in supported_models().iter().filter(|m| !is_native(m)) {
        let params = defaults_for(model);
        let _ = build_amp_processor_for_layout(model, &params, 48_000.0, AudioChannelLayout::Mono);
    }
}
