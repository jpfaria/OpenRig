...
impl LocalDispatcher {
    /// `SettingsCommand::SetLanguage` — records the language-change intent and
    ...
    pub(crate) fn handle_set_language(&self, cmd: Command) -> Result<Vec<Event>> {
        match cmd {
            Command::Settings(SettingsCommand::SetLanguage { language }) => {
                Ok(vec![Event::LanguageChanged { language }])
            }
