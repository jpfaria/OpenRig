//! Responsibility: registers the impulse response models this crate exposes.
...
#[derive(Clone, Copy)]
#[allow(dead_code)]
pub struct IrModelDefinition {
    pub id: &'static str,
    pub display_name: &'static str,
    pub brand: &'static str,
    pub backend_kind: IrBlockBackendKind,
...
pub fn find_model_definition(model: &str) -> Result<&'static IrModelDefinition> {
    MODEL_DEFINITIONS
        .iter()
        .find(|definition| definition.id == model)
        .ok_or_else(|| anyhow!("unsupported ir model '{}'", model))
}
