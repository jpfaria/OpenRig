...
/// Like [`desc`] but with the low-end ratio set too.
fn desc_full(mud: f32, fizz: f32, clip: f32, boom: f32) -> ToneDescriptors {
    ToneDescriptors {
        boom_ratio: boom,
        ..desc(mud, fizz, clip)
    }
...
#[test]
fn boom_limit_is_calibrated_too() {
    let samples = repeat("prog", desc_full(0.4, 0.1, 0.0, 0.25), 3);
    let profiles = calibrate(&samples, DEFAULT_PERCENTILE);
    assert_eq!(profiles.len(), 1);
    assert!(
        (profiles[0].boom_limit - 0.25).abs() < 1e-6,
        "boom limit calibrated: {:?}",
        profiles[0]
    );
}
